...
class PropertyMiddleware
{
    public function handle(Request $request, Closure $next)
    {
        ...
        try {
            ...
            $request->user = Property::where('status','!=',3)->find($decoded->sub);
            if($this->CheckToken($request->user->id,$token)) {

             if ($request->user->status==3 || $request->user->status==4)
            {
                return response()->json([
                  'status' => 'error',
                  'message' => 'User property is deleted. Please contact to admin',
                  'code' => 401
                ]);
            }
          }
