                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            ...
                                            <select class="form-control custom-select-icon @error('category_id') is-invalid @enderror" name="category_id">
                                                <option value="">Select Role</option>
                                                @if($get_category)
                                                    @foreach ($get_category as $category)
                                                        <option value="{{ $category->id }}" @if(empty($hotel)) {{ old('category_id') == $category->id ? 'selected' : '' }} @else {{ (isset($hotel) && $hotel->category_id == $category->id) ? 'selected' : '' ; }} @endif>{{ $category->title }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            @error('category_id')
